...
with tab2:
    ...
    if st.button("Predict Cluster"):
        ...
        cluster = int(kmeans.predict(new_data)[0])

        median_income = df['Annual Income (k$)'].median()
        median_spending = df['Spending Score (1-100)'].median()
        cluster_avg_income = cluster_summary.loc[cluster, 'Avg_Income']
        cluster_avg_spending = cluster_summary.loc[cluster, 'Avg_Spending']

        high_income = cluster_avg_income >= median_income
        high_spending = cluster_avg_spending >= median_spending

        if high_income and high_spending:
            tier = "Premium Membership"
            profile = "High income, high spending customers"
            strategy = "Offer priority access, premium loyalty rewards, and early access to new products."
        elif high_income and not high_spending:
            tier = "Exclusive Perks Membership"
            profile = "High income, low spending customers"
            strategy = "Provide personalized offers and incentives to encourage increased spending."
        elif not high_income and high_spending:
            tier = "Discount Membership"
            profile = "Low income, high spending customers"
            strategy = "Offer value-based discounts and instalment options to retain loyalty."
        else:
            tier = "Standard Membership"
            profile = "Low income, low spending customers"
            strategy = "Provide entry-level offers and engagement campaigns to build long-term loyalty."

        st.markdown(
            f"""
            <div style="border:1px solid #d0d0d0; border-radius:8px; padding:20px; background-color:#f8f9fa; margin-top:10px;">
                <h4 style="margin-bottom:5px;">Cluster {cluster} &mdash; {tier}</h4>
                <p style="margin:4px 0;"><strong>Customer Profile:</strong> {profile}</p>
                <p style="margin:4px 0;"><strong>Recommended Strategy:</strong> {strategy}</p>
                <hr style="margin:10px 0;">
                <p style="margin:2px 0;"><strong>Annual Income:</strong> ${income}k</p>
                <p style="margin:2px 0;"><strong>Spending Score:</strong> {spending}/100</p>
            </div>
            """,
